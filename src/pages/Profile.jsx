  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: requests } = await supabase
        .from('requests')
        .select('*, listing:listings(title, photo_url, urgency)')
        .eq('seeker_id', user.id)
        .order('created_at', { ascending: false });

      // Fetch my bridges (Mediator)
      ...
      <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
          <div className="flex items-center gap-4 mb-4">
            <span className="text-xs tracking-[0.2em] uppercase font-bold text-secondary">{role} Dashboard</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 space-y-16">
          ...
          {role === 'donor' && (
            <section>
              <div className="flex justify-between items-center mb-8 border-b border-outline-variant/20 pb-4">
                <h2 className="text-xl font-serif text-on-surface/80 tracking-tight">Your Contributions</h2>
                    ...
                    <Link key={item.id} to={`/listing/${item.id}`} className="group block">
                      <div className="flex gap-6 bg-surface-container-lowest p-6 rounded-lg transition-all duration-300 hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.05)] border border-transparent hover:border-outline-variant/20">
                        ...
                        <div className="ml-auto flex flex-col items-end justify-center">
                           <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-widest rounded-sm ${item.status === 'active' ? 'bg-secondary/10 text-secondary' : 'bg-surface-container-high text-on-surface-variant'}`}>{item.status}</span>
                        </div>
                      </div>
                    </Link>
                  ))
          ...
          {role === 'seeker' && (
            <section>
              <div className="flex justify-between items-center mb-8 border-b border-outline-variant/20 pb-4">
                <h2 className="text-xl font-serif text-on-surface/80 tracking-tight">Your Procured Pieces</h2>
              </div>
          ...
          {role === 'mediator' && (
            <section>
              <div className="flex justify-between items-center mb-8 border-b border-outline-variant/20 pb-4">
                <h2 className="text-xl font-serif text-on-surface/80 tracking-tight">Your Escrow History</h2>
              </div>
              
              <div className="space-y-6">
                {myBridges.length === 0 ? (
                  <div className="bg-surface-container-low p-8 rounded-lg text-center border-l-2 border-outline-variant/20">
                    <p className="text-sm text-on-surface-variant leading-relaxed">You haven't completed any bridge assignments yet.</p>
                  </div>
                ) : (
                  myBridges.map(req => (
                    <div key={req.id} className="flex justify-between items-center bg-surface-container-lowest p-6 rounded-lg border border-outline-variant/20">
                      <div>
                        <h4 className="text-md font-serif mb-1">{req.listing?.title || 'Unknown Piece'}</h4>
                        <p className="text-[10px] text-on-surface-variant uppercase tracking-widest font-bold">Bridge {req.id.split('-')[0]}</p>
                      </div>
                      <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-widest rounded-sm ${req.status === 'completed' ? 'bg-secondary/10 text-secondary' : 'bg-tertiary-container text-on-tertiary-container'}`}>
                        {req.status}
                      </span>
                    </div>
                  ))
          ...
          <div className="sticky top-32 space-y-8">
            <div className="bg-primary-container text-on-primary p-10 rounded-lg relative overflow-hidden">
              <div className="relative z-10">
                <p className="text-[10px] text-on-primary-fixed-variant uppercase tracking-widest font-bold mb-6">Impact Metrics</p>
                ...
                <div className="space-y-6">
                  {role === 'donor' && (
                    <div className="flex justify-between items-center text-[10px] tracking-widest border-b border-white/10 pb-4">
                      <span className="opacity-60 uppercase font-bold">PIECES DONATED</span>
                  ...
                  {role === 'mediator' && (
                    <div className="flex justify-between items-center text-[10px] tracking-widest border-b border-white/10 pb-4">
                      <span className="opacity-60 uppercase font-bold">BRIDGES COVERED</span>
                      <span className="font-serif text-2xl text-secondary-fixed">{myBridges.length}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center text-[10px] tracking-widest border-b border-white/10 pb-4">
                    <span className="opacity-60 uppercase font-bold">IMPACT SCORE</span>
                    <span className="font-serif text-2xl text-secondary-fixed">{profile?.rating || 0}</span>
                  </div>
                </div>
              </div>
